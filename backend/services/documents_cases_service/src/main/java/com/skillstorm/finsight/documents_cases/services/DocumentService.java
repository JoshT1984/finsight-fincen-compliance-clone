...
public class DocumentService {
	...
	public DocumentService(DocumentRepository repo, S3Service s3Service, CaseFileRepository caseFileRepo,
			ComplianceEventServiceClient complianceEventClient, AuditEventService auditEventService,
			DocumentEventPublisher documentEventPublisher, PdfExtractionService pdfExtractionService,
	...
	public DocumentResponse create(CreateDocumentRequest dto) {
		...
		if (dto.documentType() == DocumentType.CTR && finalCtrId == null) {
			...
			CreateCtrRequestPayload payload = new CreateCtrRequestPayload("PDF_IMPORT", sourceEntityId, null, now,
					BigDecimal.ZERO, "(No file)", now, Map.of());
			...
			String sourceEntityId = "DOC_" + UUID.randomUUID();
			Instant now = Instant.now();
			CreateSarRequestPayload payload = new CreateSarRequestPayload("PDF_IMPORT", sourceEntityId, null, now, null,
					"(No file)", null, null, Map.of(), 50);
		...
		eventEmitter.emit(
				com.skillstorm.finsight.documents_cases.loggers.DocumentEventLog.documentCreated(
						saved.getDocumentId().toString(),
						...
						java.util.Map.of(
								...
								"uploadedByUserId", saved.getUploadedByUserId())));
	...
	public List<DocumentResponse> findAll() {
		...
		log.warn(
				"Document findAll denied: user has no recognized role (ANALYST, LAW_ENFORCEMENT_USER, COMPLIANCE_USER)");
		return List.of();
	}
	...
	public List<DocumentResponse> findByCaseId(Long caseId) {
		...
		if (SecurityContextUtils.isLawEnforcement()) {
			...
			// Include CASE documents (case_id) and SAR documents tied to this case (sar_id
			// = case.sarId)
		...
		if (SecurityContextUtils.isComplianceUser()) {
			...
			CaseFile caseFile = caseFileRepo.findById(caseId).orElse(null);
			if (caseFile == null)
				return List.of(); // Case does not exist
			List<Document> byCase = repo.findByCaseId(caseId);
			List<Document> bySar = repo.findBySarId(caseFile.getSarId());
			return java.util.stream.Stream.concat(byCase.stream(), bySar.stream())
					.distinct()
		...
		if (SecurityContextUtils.isAnalyst()) {
			CaseFile caseFile = caseFileRepo.findById(caseId).orElse(null);
			if (caseFile == null)
				return repo.findByCaseId(caseId).stream().map(this::toResponse).collect(Collectors.toList());
			List<Document> byCase = repo.findByCaseId(caseId);
			List<Document> bySar = repo.findBySarId(caseFile.getSarId());
			return java.util.stream.Stream.concat(byCase.stream(), bySar.stream())
					.distinct()
	...
	public DocumentResponse updateById(Long documentId, UpdateDocumentRequest request) throws IOException {
		...
		if (newDocumentType == DocumentType.CTR) {
			...
			// For SAR documents, validate case exists if caseId is provided (same as create
			// method)
			// Only validate if caseId is being changed (if unchanged, it was already
	...
	public void deleteById(Long documentId) {
		...
		try {
			...
			log.error(
					"CRITICAL: Failed to delete file from S3 for document ID: {} (storagePath: {}). Error: {}. Continuing with database deletion.",
	 ...
	 */
	@Transactional
	public DocumentResponse upload(MultipartFile file, DocumentType documentType, Long ctrId, Long sarId, Long caseId)
			throws IOException {
		...
		// - CTR documents: ctrId optional (will be created if not provided), no sarId
		// or caseId
		// - SAR documents: sarId optional (will be created if not provided), caseId
		...
		try {
			...
			// Emit event for document upload, type-specific
			String userId = SecurityContextUtils.getCurrentUserId().orElse(null).toString();
			java.util.Map<String, Object> eventMeta = java.util.Map.of(
					"documentType", saved.getDocumentType(),
					"fileName", saved.getFileName(),
					"storagePath", saved.getStoragePath(),
					"uploadedAt", saved.getUploadedAt(),
					...
					"s3Key", s3Key);
			switch (saved.getDocumentType()) {
				case CTR -> eventEmitter.emit(
						com.skillstorm.finsight.documents_cases.loggers.DocumentEventLog.documentCtrUploaded(
								saved.getDocumentId().toString(), userId, eventMeta));
				case SAR -> eventEmitter.emit(
						com.skillstorm.finsight.documents_cases.loggers.DocumentEventLog.documentSarUploaded(
								saved.getDocumentId().toString(), userId, eventMeta));
				case CASE -> eventEmitter.emit(
						com.skillstorm.finsight.documents_cases.loggers.DocumentEventLog.documentCaseUploaded(
								saved.getDocumentId().toString(), userId, eventMeta));
				default -> eventEmitter.emit(
						com.skillstorm.finsight.documents_cases.loggers.DocumentEventLog.documentUploaded(
								saved.getDocumentId().toString(), userId, eventMeta));
			}
			...
			// If database save fails (constraint violation, etc.), clean up the S3 file to
	 ...
	 */
	private String generateS3Key(DocumentType documentType, String originalFilename, Long ctrId, Long sarId,
			Long caseId) {
		String prefix;
		Long id;
	...
	/**
	 * Validates that the uploaded file is a PDF when creating a new CTR or SAR from
	 * extraction.
	 */
	private void validatePdfForUpload(MultipartFile file, String documentTypeLabel) {
		...
		if (!contentTypePdf && !extensionPdf) {
			throw new IllegalArgumentException(
					documentTypeLabel + " creation from upload requires a PDF file. Got content type: " + contentType
							+ ", filename: " + name);
		}
	}
